...
func InitHttpProtocol(mode string) (*http.HttpImpl, error) {
	...
	cartRepositoryImpl, err := cart.ProvideRepository(databaseImpl)
	if err != nil {
		return nil, err
	}
	cartServiceImpl, err := cart.ProvideService(cartRepositoryImpl, productRepositoryImpl)
	if err != nil {
		return nil, err
	}
	cartHandlerImpl, err := cart.ProvideHandler(cartServiceImpl)
	if err != nil {
		return nil, err
	}
	orderRepositoryImpl, err := order.ProvideRepository(databaseImpl)
	if err != nil {
		return nil, err
	}
	orderServiceImpl, err := order.ProvideService(orderRepositoryImpl, cartRepositoryImpl, jwtTokenImpl)
	if err != nil {
		return nil, err
	}
	orderHandlerImpl, err := order.ProvideHandler(orderServiceImpl)
	if err != nil {
		return nil, err
	}
	...
	httpHandlerImpl := handler.NewHttpHandler(userHandlerImpl, productHandlerImpl, cartHandlerImpl, orderHandlerImpl, healthHandlerImpl)
